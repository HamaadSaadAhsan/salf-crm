root@pbx:/var/www/salf-crm# cat /opt/incoming.php
#!/usr/bin/php -q
<?php
/**
 ...
 *
 * Routing Logic:
 * 1. If lead exists and is assigned to a CRO with an extension, route to that CRO
 * 2. Otherwise, find an available CRO using weighted round-robin algorithm:
 *    - availability = true
 *    - has 'cro' role (or support-agent/senior-support-agent)
 *    - has a valid extension
 *    - scoring based on workload, performance_weight, conversion_rate, recency
 * 3. If no CRO available, route to default queue extension '299'
 */
...
try {
    ...
    if ($lead) {
        ...
        $agi->verbose("No existing lead found for phone: $callerId", 1);
    }

    // Determine CRO to route the call to
    $targetCRO = null;
    $targetExtension = DEFAULT_EXTENSION; // Default to ring group
    $routingDecision = 'fallback_queue';
    $routingReason = 'No CRO available, routing to ring group';
    $routingScore = null;

    // Step 1: If lead has an assigned CRO, try to route to them
    if ($assignedTo) {
        $assignedCRO = getUserById($dbConnection, $assignedTo);

        if ($assignedCRO && ! empty($assignedCRO['extension'])) {
            // Calculate score for logging purposes
            $assignedScore = calculateAssignmentScore($assignedCRO);

            $targetCRO = $assignedCRO;
            $targetExtension = $assignedCRO['extension'];
            $routingDecision = 'assigned_cro';
            $routingReason = "Lead is assigned to CRO {$assignedCRO['name']} (ID: {$assignedCRO['id']}, score: {$assignedScore})";
            $routingScore = $assignedScore;

            $agi->verbose("Routing to assigned CRO: {$assignedCRO['name']} at extension $targetExtension (score: $assignedScore)", 1);
        } else {
            $agi->verbose('Assigned CRO has no extension or not found, searching for available CRO', 1);
        }
    }
    ...
    // intended_for_user_id is the lead's assigned CRO (if exists)
    // This helps track coverage calls when another CRO answers
    $intendedForUserId = $assignedTo ?: $userId;
    ...
    // Insert routing decision log with weighted round-robin details BEFORE any events are dispatched
    insertCallLog(
        $dbConnection,
        $callSessionId,
        'info',
        ...
        [
            ...
            'target_cro_workload' => $targetCRO ? ($targetCRO['current_lead_count'] ?? 0) : null,
            'target_cro_performance_weight' => $targetCRO ? ($targetCRO['performance_weight'] ?? 1.0) : null,
            'target_cro_conversion_rate' => $targetCRO ? ($targetCRO['conversion_rate'] ?? 0) : null,
            'lead_id' => $leadId,
    ...
    // Set recording path based on call signature
    $callRec = "/var/www/html/recordz/{$callSignature}";
