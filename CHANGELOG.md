...
### Added
- Laravel Fortify is now the authentication backend (replacing the Breeze auth controllers). Fortify's view callbacks in `FortifyServiceProvider` render the existing Inertia auth pages, so the login/forgot/reset/verify/confirm screens are unchanged. All Fortify features are enabled: password reset, email verification, password confirmation, profile/password updates, two-factor authentication, and passkeys. New `two_factor_*` columns on `users` and a `passkeys` table; `User` implements `Laravel\Passkeys\Contracts\PasskeyUser` and uses the `PasskeyAuthenticatable` + `TwoFactorAuthenticatable` traits.
- Passkey and two-factor frontend (Phase 2). "Sign in with a passkey" button on the login page (shown only when the browser supports passkeys) runs the assertion ceremony via the first-party `@laravel/passkeys` client. New `settings/security` page (Settings → Security) lists registered passkeys and lets users add/remove them and enable/disable TOTP two-factor authentication with QR enrolment and recovery codes; passkey and 2FA management actions are gated behind Fortify password confirmation. New `auth/two-factor-challenge` page handles the authenticator-code / recovery-code step at login. Browser WebAuthn helpers live in `resources/js/lib/passkey.ts` and `resources/js/lib/two-factor.ts`; the page is served by `Settings\SecurityController` at the `settings.security` route.
...
### Fixed
- Security settings page reported "This browser does not support passkeys" in WebAuthn-capable browsers: `browserSupportsWebAuthn()` was called during render, where Inertia SSR has no `window` and the value never recomputed on the client. It now resolves after mount via `useEffect`, matching the login page. Passkey login also now navigates to the server-provided `redirect` (honouring the intended URL) instead of a hardcoded `/dashboard`, and `passkeys.redirect` defaults to `/dashboard`.
- SSR crash (`window is not defined`) on the Services and Users index pages: both read `window.location.search` in the render body. They now derive the initial search term from Inertia's `usePage().url`, which is identical on server and client (no hydration mismatch).
- Passkey registration and login were rejected with an origin mismatch when the site is served over HTTPS but `APP_URL` uses `http` — the WebAuthn origin check compared the browser's `https://…` origin against the `http://…` entry in `allowed_origins`. `passkeys.allowed_origins` now accepts both the `http` and `https` variants of the app host (WebAuthn only runs in secure contexts, so allowing the `http` origin is harmless). The passkey browser client was also switched from a hand-rolled `@simplewebauthn/browser` wrapper to the first-party `@laravel/passkeys` package, which surfaces typed errors (`UserCancelledError`, `PasskeyExistsError`, `InvalidDomainError`, …) for clearer failure messages.
