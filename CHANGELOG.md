...
### Changed
- Added the official `@inertiajs/vite` plugin (Inertia v3.3) alongside `@vitejs/plugin-react`: it auto-resolves Inertia page components and the SSR/app entries, so the explicit `ssr` input on the `laravel()` plugin is no longer needed. `@vitejs/plugin-react` is retained — it still provides the JSX transform and the dev `/@react-refresh` HMR runtime. Bumped `@inertiajs/react` to `^3.3.1` and added `@inertiajs/vite` `^3.3.1`. Removed the now-redundant `use_script_element_for_initial_page` flag from `config/inertia.php` (handled by the plugin)
