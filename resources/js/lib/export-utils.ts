...
export async function exportToPDF(
    ...
    try {
        ...
        pdf.addImage(imgData, 'PNG', imgX, options?.title ? imgY + 10 : imgY, imgWidth * ratio, imgHeight * ratio);
 ...
 */
export function formatDataForExport(data: any[], format: 'csv' | 'json' | 'table'): string {
    switch (format) {
        case 'csv': {
            const headers = Object.keys(data[0]);
        ...
        case 'table': {
            ...
            const maxLengths = keys.map((key) => Math.max(key.length, ...data.map((row) => String(row[key]).length)));

            const header = keys.map((key, i) => key.padEnd(maxLengths[i])).join(' | ');

            const separator = maxLengths.map((len) => '-'.repeat(len)).join('-+-');

            const rows = data.map((row) => keys.map((key, i) => String(row[key]).padEnd(maxLengths[i])).join(' | '));
