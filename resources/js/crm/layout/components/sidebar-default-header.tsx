...
export function SidebarCollapseButton({ collapsed, onClick, className }: { collapsed: boolean; onClick: (e: React.MouseEvent) => void; className?: string }) {
    ...
    return (
        <button
            ...
            className={`flex items-center justify-center size-6 shrink-0 cursor-pointer bg-transparent border-none outline-none p-0 text-sidebar-foreground ${className ?? ''}`}
            onClick={onClick}
            ...
            >
                ...
                <path
                    ...
                    style={{
                        ...
                        transform: hovering ? 'translateX(1px) scaleX(0.8)' : 'translateX(0) scaleX(1)',
...
export function SidebarDefaultHeader() {
    ...
    return (
        <div className="group flex h-11 shrink-0 items-center justify-between gap-2.5 border-b border-border px-2.5 lg:h-(--sidebar-header-height)">
            ...
            <SidebarCollapseButton collapsed={sidebarCollapse} onClick={() => setSidebarCollapse(!sidebarCollapse)} className="hidden lg:group-hover:flex lg:in-data-[sidebar-collapsed]:hidden!" />
        </div>
    );
}
