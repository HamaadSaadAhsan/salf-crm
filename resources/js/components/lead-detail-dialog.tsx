...
import { useInfiniteLeadAllActivities, useInfiniteLeadComments, useLead, useOptimisticLeadUpdate } from '@/hooks/useLead';
...
const FilePreview = ({ attachment }: FilePreviewProps) => {
    ...
    const getPreviewContent = () => {
        if (mime_type.startsWith('image/')) {
            return (
                <div className="space-y-2">
                    <div className="max-w-xs rounded-md bg-gray-700 p-2">
                        <img
                            src={`/storage/${file_path}`}
                            alt={original_name}
                            className="h-auto max-h-55 w-full rounded"
                            onError={(e) => {
                        ...
                        />
                        <div style={{ display: 'none' }} className="flex h-24 items-center justify-center text-gray-400">
                            <span>Preview not available</span>
                        </div>
                    </div>
                    <div className="px-2 py-2">
                        <p className="text-sm font-semibold">{original_name}</p>
        ...
        if (mime_type === 'application/pdf') {
            return (
                <div className="space-y-2">
                    <div className="max-w-xs rounded-md bg-gray-700 p-4">
                        <div className="flex h-24 items-center justify-center">
                            <FileText className="h-12 w-12 text-red-500" />
                        </div>
                        <p className="mt-2 text-center text-sm">PDF Document</p>
                    </div>
            ...
            return (
                <div className="space-y-2">
                    <div className="max-w-xs rounded-md bg-gray-700 p-4">
                        ...
                        <p className="mt-2 text-center text-sm">Video File</p>
                    </div>
                    <div className="space-y-1">
                        <p className="text-sm font-semibold">{original_name}</p>
                        <p className="text-xs text-gray-300">
    ...
    return (
        <div className="group relative">
            ...
            {canScrollLeft && (
                <button
                    onClick={() => scroll('left')}
                    className="absolute top-1/2 left-0 z-10 -translate-y-1/2 rounded-full bg-gray-800/80 p-1 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-gray-700/80"
                >
            ...
            {canScrollRight && (
                <button
                    ...
                    className="absolute top-1/2 right-0 z-10 -translate-y-1/2 rounded-full bg-gray-800/80 p-1 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-gray-700/80"
                >
            ...
            <div
                ...
                className="scroll-container flex gap-2 overflow-x-auto px-1 py-2"
                style={{
                    scrollbarWidth: 'none',
                    ...
                    <TooltipProvider key={idx} delayDuration={0}>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <div className="flex min-w-fit flex-shrink-0 cursor-pointer items-center gap-2 rounded-md bg-gray-800 px-3 py-2 transition-colors hover:bg-gray-700">
                                    {getFileIcon(attachment.mime_type)}
                                    <span className="max-w-24 truncate text-xs whitespace-nowrap text-blue-400">{attachment.original_name}</span>
                                </div>
                            </TooltipTrigger>
                            <TooltipContent side="top" className="max-w-xs">
                                <div className="space-y-1">
                                    <p className="text-sm font-semibold">{attachment.original_name}</p>
                                    <p className="text-gray-foreground text-xs">
...
export function LeadDetailDialog({
    ...
    const handleScroll = useCallback(
        (scrollElement: HTMLDivElement, hasNextPage: boolean | undefined, fetchNextPage: () => void, isFetchingNextPage: boolean) => {
        ...
        const setupListeners = () => {
            ...
            const commentsScrollHandler = () => {
                if (commentsElement && activeTab === 'comments') {
                    handleScroll(commentsElement, commentsQuery.hasNextPage, commentsQuery.fetchNextPage, commentsQuery.isFetchingNextPage);
                }
            };
    ...
    const debouncedUpdateDetails = useCallback(
        (leadId: string, newDescription: string) => {
        ...
        [updateLead, queryClient],
    ); // Remove lead?.detail from dependencies
    ...
    useEffect(() => {
        ...
        const matchedTags = leadTags.filter((tag) => leadTagValues.includes(tag.value));

        // Only update if tags actually changed
        setSelectedTags((prevTags) => {
            const prevValues = prevTags
                .map((t) => t.value)
                .sort()
                .join(',');
            const newValues = matchedTags
                .map((t) => t.value)
                .sort()
                .join(',');
    ...
    if (isLoading) {
        return (
            <Sheet open={isOpen} onOpenChange={onOpenChangeAction}>
                <SheetContent className="sm:max-w-1xl flex h-full w-full flex-col gap-0 overflow-y-auto border-gray-800 bg-gray-900 p-0 text-white md:max-w-2xl lg:max-w-3xl [&>button]:hidden">
    ...
    const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Array.from(files).forEach((file) => {
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = (e) => {
                    setAttachedFiles((prev) => prev.map((f) => (f.id === fileId ? { ...f, preview: e.target?.result as string } : f)));
                };
    ...
    return (
        <Sheet open={isOpen} onOpenChange={onOpenChangeAction}>
            <SheetContent className="sm:max-w-1xl flex h-full w-full flex-col gap-0 border-gray-800 bg-gray-900 p-0 text-white md:max-w-2xl lg:max-w-3xl [&>button]:hidden">
                ...
                <div className="flex-grow overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between border-b border-gray-800 p-3">
                        <div className="flex items-center gap-1">
                            ...
                            <Button
                                ...
                                className="h-8 w-8 text-gray-400 hover:bg-gray-800 hover:text-white disabled:opacity-30"
                            >
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <PortalContainerProvider container={containerRef.current}>
                                <LeadDialogMoreMenu handleTagsInputsShow={(value: boolean) => handleTagInputsShow(value)} />
                    ...
                    <div className="px-4 pb-4">
                        <div className="flex items-center-safe gap-2">
                            <h1 className="mb-1 text-xl font-semibold text-white">{lead?.name}</h1>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 px-4">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="w-20 flex-shrink-0 text-sm text-gray-300"></span>
                            <Button
                                variant="ghost"
                                className="h-auto cursor-pointer px-1 py-0 text-sm font-normal text-gray-400 hover:text-yellow-500"
                            >
                    ...
                    <div className="space-y-4 px-4 pb-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                                <span className="w-16 flex-shrink-0 text-sm text-gray-400">City</span>
                            ...
                            <div className="flex items-center gap-3">
                                <span className="w-16 flex-shrink-0 text-sm text-gray-400">Country</span>
                                <div className="flex items-center gap-2">
                                    <span className="text-sm text-white">{getCountryDisplayName(lead?.country || '')}</span>
                                </div>
                            </div>

                            {/* Source */}
                            <div className="flex items-center gap-3">
                                <span className="w-16 flex-shrink-0 text-sm text-gray-400">Source</span>
                                <div className="flex items-center gap-2">
                                    {getSourceProperty('slug') === 'facebook-ads' && <IconBrandFacebook className="h-4 w-4 text-blue-600" />}
                                    {getSourceProperty('slug') === 'google-ads' && <IconBrandGoogle className="h-4 w-4 text-red-500" />}
                                    {getSourceProperty('slug') === 'linkedin' && <IconBrandLinkedin className="h-4 w-4 text-blue-700" />}
                                    {getSourceProperty('slug') === 'cold-call' && <Phone className="h-4 w-4 text-blue-500" />}
                                    {getSourceProperty('slug') === 'email-campaign' && <Mail className="h-4 w-4 text-purple-500" />}
                                    {getSourceProperty('slug') === 'website-contact-form' && <Globe className="h-4 w-4 text-cyan-500" />}
                            ...
                            {/* Priority */}
                            <div className="flex items-center gap-3">
                                <span className="w-16 flex-shrink-0 text-sm text-gray-400">Priority</span>
                                <div className="flex items-center gap-2">
                                    {lead?.priority === 'urgent' && <CircleArrowOutUpRightIcon size={16} className="text-red-500" />}
                                    {lead?.priority === 'high' && <ChevronUpCircleIcon size={16} className="text-orange-500" />}
                                    {lead?.priority === 'medium' && <ChevronRightCircleIcon size={16} className="text-green-500" />}
                                    {lead?.priority === 'low' && <ChevronDownCircleIcon size={16} className="text-cyan-400" />}
                                    <span className={`text-sm capitalize ${getPriorityColor(lead?.priority || '')}`}>{lead?.priority}</span>
                                </div>
                            </div>
                            ...
                            {lead?.is_hot_lead && (
                                <div className="flex items-center gap-3">
                                    <span className="w-16 flex-shrink-0 text-sm text-gray-400">Hot Lead</span>
                                    <div className="flex items-center gap-2">
                            ...
                            {lead?.labels && lead?.labels.length > 0 && (
                                <div className="flex items-start gap-3">
                                    <span className="w-16 flex-shrink-0 text-sm text-gray-400">Labels</span>
                                    <div className="flex flex-wrap gap-2">
                                        {lead?.labels.map((label: string, index: number) => (
                                            <Badge key={index} variant="secondary" className="border-gray-700 bg-gray-800 text-gray-300">
                                                {label}
                                            </Badge>
                                        ))}
                                    </div>
                    ...
                    <div className="bg-gray-800 pt-5">
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col">
                            <TabsList className="mx-4 grid w-fit grid-cols-2 rounded-none border-gray-800 bg-transparent p-0">
                                <TabsTrigger
                                    value="comments"
                                    className="mr-4 rounded-none px-0 pt-2 pb-3 text-sm font-medium text-gray-400 data-[state=active]:border-b-2 data-[state=active]:border-white data-[state=active]:bg-transparent data-[state=active]:text-white data-[state=active]:shadow-none dark:data-[state=active]:border-t-0 dark:data-[state=active]:border-r-0 dark:data-[state=active]:border-l-0 dark:data-[state=active]:border-b-primary dark:data-[state=active]:bg-transparent"
                                >
                                    Comments
                                </TabsTrigger>
                                <TabsTrigger
                                    value="activity"
                                    className="rounded-none px-0 pt-2 pb-3 text-sm font-medium text-gray-400 data-[state=active]:border-b-2 data-[state=active]:border-white data-[state=active]:bg-transparent data-[state=active]:text-white data-[state=active]:shadow-none dark:data-[state=active]:border-t-0 dark:data-[state=active]:border-r-0 dark:data-[state=active]:border-l-0 dark:data-[state=active]:border-b-primary dark:data-[state=active]:bg-transparent"
                                >
                                    All activity
                                </TabsTrigger>
                            </TabsList>
                            ...
                            <TabsContent value="comments" className="mt-0 px-0 py-0">
                                <div ref={commentsScrollRef} className="max-h-96 overflow-y-auto px-4 py-4">
                                    {(() => {
                                            ...
                                            <div className="space-y-4">
                                                {allCommentsData.map((activity: LeadActivity, index: number) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className={`flex max-w-[80%] gap-3 ${isAssignee ? 'flex-row' : 'flex-row-reverse'}`}>
                                                                ...
                                                                <div className={`flex flex-col ${isAssignee ? 'items-start' : 'items-end'}`}>
                                                                    <div
                                                                        className={`max-w-full rounded-2xl px-4 py-2 break-words ${
                                                                            isAssignee ? 'bg-gray-700 text-white' : 'bg-blue-600 text-white'
                                                                        }`}
                                                                    >
                                                                                ...
                                                                                {activity.attachments.map((attachment: any, attIdx: number) => (
                                                                                    <TooltipProvider key={attIdx} delayDuration={100}>
                                                                                        <Tooltip>
                                                                                            ...
                                                                                            <TooltipContent
                                                                                                side="top"
                                                                                                className="max-w-md border-gray-600 p-0"
                                                                                            >
                                                                    ...
                                                                    <div
                                                                        className={`mt-1 flex items-center gap-2 text-xs text-gray-400 ${isAssignee ? 'flex-row' : 'flex-row-reverse'}`}
                                                                    >
                            ...
                            <TabsContent value="activity" className="mt-0 px-0 py-0">
                                <div ref={activitiesScrollRef} className="max-h-96 overflow-y-auto px-4 py-4 text-sm text-gray-400">
                                    {(() => {
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <Avatar className="h-8 w-8">
                                                        <AvatarImage src="/placeholder.svg?height=32&width=32" />
                                                        <AvatarFallback className="bg-blue-600 text-sm text-white">
                                                            {lead?.created_by?.name?.[0]}
                                                        </AvatarFallback>
                                                    </Avatar>
                                                    <div>
                                                        <p className="text-sm text-white">
                                                            <span className="font-semibold">{lead?.created_by?.name}</span> created this lead &middot;{' '}
                                                            {lead?.created_at}
                                                        </p>
                                                    </div>
                                                </div>
                                                                            ...
                                                                            {activity.attachments && activity.attachments.length === 1 ? (
                                                                                <>
                                                                                    ...
                                                                                    <TooltipProvider delayDuration={100}>
                                                                                        <Tooltip>
                                                                                            <TooltipTrigger asChild>
                                                                                                <span className="cursor-pointer text-blue-400 underline decoration-dotted underline-offset-2 hover:text-blue-300">
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        <Collapsible>
                                                                            <CollapsibleTrigger asChild>
                                                                                <Button
                                                                                    variant="ghost"
                                                                                    className="h-auto p-2 text-xs text-blue-400 hover:bg-gray-800 hover:text-blue-300"
                                                ...
                                                {allActivitiesQuery.isFetchingNextPage && (
                                                    <div className="flex items-center justify-center py-4">
                                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                                        <span className="text-sm text-gray-400">Loading more activities...</span>
                                                    </div>
                                                )}

                                                {/* No activities message */}
                                                {allActivitiesData.length === 0 && !allActivitiesQuery.isLoading && (
                                                    <div className="ml-11 text-sm text-gray-400">No activities yet.</div>
                                                )}
                                            </div>
                                        );
                                    })()}
                ...
                <SheetFooter className="space-y-4 border-t border-gray-800 p-4">
                    <div className="flex items-start gap-3">
                        <Avatar className="h-8 w-8">
                            <AvatarImage src="/placeholder.svg?height=32&width=32" />
                            <AvatarFallback className="bg-blue-600 text-sm text-white">{lead?.assigned_to?.name[0]}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1">
                            <Textarea
                                ref={commentBoxRef}
                                ...
                                className="min-h-[80px] resize-none border-gray-700 bg-gray-800 text-white placeholder:text-gray-500"
                            />

                            {/* File Attachments */}
                            {attachedFiles.length > 0 && (
                                <div className="py-2">
                                    <div ref={containerRef} className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                                        <PortalContainerProvider container={containerRef.current}>
                            ...
                            <div className="mt-2 flex items-center justify-between">
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <Button
                                                    ...
                                                    className="h-8 w-8 cursor-pointer text-gray-400 hover:bg-blue-900 hover:text-white dark:hover:bg-blue-900"
                                                >
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                        ...
                                        className="h-auto bg-blue-600 px-4 py-1.5 text-sm text-white hover:bg-blue-700 disabled:opacity-50"
                                    >
                                        Comment
                                    </Button>
                                </div>
