...
export default function LeadFollowupPicker({ lead }: { lead: Lead | null | undefined }) {
    ...
    const [date, setDate] = React.useState<Date | undefined>(lead?.next_follow_up_at ? new Date(lead?.next_follow_up_at) : undefined);
    ...
    return (
        <div className="flex items-center gap-3">
            <span className="w-26 flex-shrink-0 text-sm text-gray-300">Next Follow Up</span>
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <Button
                        size="sm"
                        id="date"
                        className="flex cursor-pointer justify-between bg-gray-800 px-2 py-0.5 font-normal text-gray-300 transition-colors hover:bg-gray-700"
                    >
                ...
                <PopoverContent container={portalContainer} className="w-auto overflow-hidden p-0" align="start">
                    <div className="space-y-3 p-3">
                        ...
                        <div className="flex gap-2">
                            <Button onClick={handleFollowUpUpdate} disabled={!date} size="sm" className="flex-1">
                                <Check className="mr-2 h-4 w-4" />
                                Set Follow-up
                            </Button>
                            <Button onClick={handleClearFollowUp} variant="outline" size="sm" className="flex-1">
                                Clear
                            </Button>
                        </div>
                    </div>
            ...
            <Button onClick={handleClearFollowUp} variant="ghost" size="icon" className="h-6 w-6 text-gray-400">
                <X className="h-3 w-3" />
            </Button>
        </div>
    );
