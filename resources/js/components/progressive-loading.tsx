...
export const Skeleton: React.FC<SkeletonProps> = ({ className, variant = 'pulse', width, height, lines = 1, animate = true, ...props }) => {
    const baseClasses = cn('bg-gray-200 dark:bg-gray-700', animate && 'animate-pulse', className);
    ...
    return <div className={cn(baseClasses, variantClasses[variant])} style={style} {...props} />;
};

// Lead row skeleton with exact structure matching
export const LeadRowSkeleton: React.FC<{ animate?: boolean }> = ({ animate = true }) => (
    <div className="grid grid-cols-[28px_28px_28px_28px_160px_1fr_auto_auto_60px_auto_auto] items-center border-b px-4 py-3">
...
export const LeadListSkeleton: React.FC<{
    ...
    showHeader?: boolean;
    animate?: boolean;
}> = ({ rows = 10, showHeader = true, animate = true }) => (
        ...
        <div className={cn('relative overflow-hidden', className)}>
            ...
            {(placeholderSrc || blurDataURL) && !imageLoaded && (
                <img
                    ...
                    alt=""
                    className={cn('absolute inset-0 h-full w-full object-cover', blurDataURL && 'scale-110 blur-sm filter')}
                />
            )}

            {/* Loading skeleton */}
            {!imageLoaded && !imageError && !placeholderSrc && !blurDataURL && <Skeleton className="absolute inset-0 h-full w-full" />}
...
export const ProgressiveLoader: React.FC<ProgressiveLoadingProps> = ({
    ...
    if (error) {
        return <div className={cn('flex items-center justify-center p-4 text-red-600', className)}>Failed to load content</div>;
    }
...
export const NetworkAwareLoader: React.FC<{
            ...
            fallback || (
                <div className="flex items-center justify-center p-8 text-center">
                    <div className="space-y-4">
                        ...
                        <div>
                            <h3 className="font-semibold text-gray-900 dark:text-gray-100">{"You're"} offline</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Check your internet connection and try again</p>
                        </div>
                    </div>
                </div>
            )
    ...
    if (connectionType === 'slow-2g' || connectionType === '2g') {
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-2 rounded bg-yellow-50 p-2 text-sm dark:bg-yellow-900">
                    <Wifi className="h-4 w-4 text-yellow-600" />
                    <span className="text-yellow-700 dark:text-yellow-300">Slow connection detected. Content may take longer to load.</span>
                </div>
                {children}
            </div>
        );
...
export const LazyLoader: React.FC<{
    ...
    threshold?: number;
    once?: boolean;
}> = ({ children, skeleton, rootMargin = '50px', threshold = 0.1, once = true }) => {
    ...
    return <div ref={ref}>{shouldRender ? children : skeleton || <Skeleton className="h-32 w-full" />}</div>;
};
...
export const AdaptiveLoader: React.FC<{
    ...
    switch (deviceCapability) {
        ...
        case 'low':
            return <>{lowQuality || <Skeleton className="h-32 w-full" />}</>;
        default:
            return <>{children}</>;
    }
};

export { ProgressiveLoader as default, type LoadingState, type ProgressiveLoadingProps };
