...
const VirtualLeadRow = ({ index, style, data }: ListChildComponentProps) => {
    const { leads, selectedItems, hoveredLead, onToggleItem, onHover, onToggleStar, onLeadClick, isItemLoaded } = data;
    ...
    if (!isLoaded || !lead) {
        ...
        return (
            <div style={style} className="flex items-center border-b px-4 py-3">
    ...
    return <OptimizedLeadRow index={index} style={style} data={rowData} />;
};
...
export const VirtualizedLeadsList: React.FC<VirtualizedLeadsListProps> = ({
    ...
    const { leads, isLoading, isFetching, fetchNextPage, hasNextPage, error } = useAdaptiveLeads(filters, 'infinite');
    ...
    const itemData = useMemo(
        ...
        [leads, selectedItems, hoveredLead, onToggleItem, onHover, onToggleStar, onLeadClick, isItemLoaded],
    );
    ...
    // Empty state
    if (itemCount === 0) {
        return <div className="flex h-64 items-center justify-center text-gray-500">No leads found</div>;
    }
    ...
    return (
        <div className="relative h-screen w-full">
            ...
            {(isFetching || isLoadingMore) && (
                <div className="absolute right-0 bottom-0 left-0 flex items-center justify-center border-t py-2 backdrop-blur-sm">
