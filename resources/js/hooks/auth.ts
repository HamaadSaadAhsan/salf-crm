...
export const useAuth = ({ middleware, redirectIfAuthenticated }: { middleware?: string; redirectIfAuthenticated?: string }) => {
    ...
    const register = useCallback((data: { name: string; email: string; password: string; password_confirmation: string }) => {
        router.post('/register', data);
    }, []);

    const login = useCallback((data: { email: string; password: string; remember: boolean }) => {
    ...
    const resetPassword = useCallback((data: { email: string; password: string; password_confirmation: string; token: string }) => {
    ...
    const nextCookies = useCallback(() => {
        ...
        const sidebarState = window.document.cookie.split('; ').find((row) => row.startsWith('sidebar_state='));
