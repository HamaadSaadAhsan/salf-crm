    ...
    protected function fetchTasks(
        ...
        return $query->limit(100)->get()->map(function (Task $task) {
            return [
                ...
                'due_at_formatted' => $task->due_at?->format('M d, Y h:i A'),
                'completed_at' => $task->completed_at?->toISOString(),
                'completed_at_formatted' => $task->completed_at?->format('M d, Y'),
                'is_completed' => $task->completed_at !== null,
                'is_overdue' => $task->due_at && $task->due_at->isPast() && ! $task->completed_at,
